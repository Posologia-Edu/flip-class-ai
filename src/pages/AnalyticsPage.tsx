...
const AnalyticsPage = () => {
  ...
  const overallAvg = completedRooms.length > 0
    ? Math.round(completedRooms.reduce((s, r) => s + r.avgScore, 0) / completedRooms.length) : 0;
  ...
  const engagementTrend = (() => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      days[d.toISOString().slice(0, 10)] = { sessions: 0, completions: 0 };
    }
    for (const s of allSessions) {
      const key = s.created_at?.slice(0, 10);
      if (days[key]) days[key].sessions++;
      if (s.completed_at) { const ck = s.completed_at.slice(0, 10); if (days[ck]) days[ck].completions++; }
    }
    return Object.entries(days).map(([date, v]) => ({
      date: new Date(date).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }),
      "Novos Alunos": v.sessions, "Conclusões": v.completions,
    }));
  })();
  ...
  const questionAnalysis = (() => {
    ...
    return (quizData.questions as any[]).map((q: any, idx: number) => {
      ...
      for (const session of roomSessions) {
        ...
        if (answers && answers[key] !== undefined) { total++; if (answers[key] === q.correct || answers[key] === q.correctAnswer) correct++; }
      }
      const rate = total > 0 ? Math.round((correct / total) * 100) : 0;
      return { question: (q.question || q.text || `Questão ${idx + 1}`).substring(0, 50), fullQuestion: q.question || q.text || `Questão ${idx + 1}`, acertos: rate, total, correct };
    });
  })();
  ...
  const analyticsContent = (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><Users className="w-4 h-4" /> Alunos Total</div>
          <p className="font-display text-2xl font-bold text-foreground">{totalStudents}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><Target className="w-4 h-4" /> Concluídos</div>
          <p className="font-display text-2xl font-bold text-foreground">{totalCompleted}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><TrendingUp className="w-4 h-4" /> Taxa Geral</div>
          <p className="font-display text-2xl font-bold text-foreground">{overallCompletionRate}%</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><Clock className="w-4 h-4" /> Tempo Médio</div>
      ...
      {analytics.length > 0 ? (
        <div className="space-y-3 mb-8">
          ...
          {analytics.map((room) => (
            <div key={room.roomId} className="bg-card border border-border rounded-xl overflow-hidden">
              <button onClick={() => toggleRoomDetail(room.roomId)} className="w-full flex items-center justify-between p-4 hover:bg-secondary/50 transition-colors text-left">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{room.studentCount} alunos · {room.completedCount} concluídos · Média: {room.avgScore}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full ${room.completionRate >= 70 ? "bg-green-500/10 text-green-600" : room.completionRate >= 40 ? "bg-yellow-500/10 text-yellow-600" : "bg-red-500/10 text-red-500"}`}>{room.completionRate}%</span>
                  ...
                  {loadingDetail ? (
                    ...
                    <div className="space-y-6">
                      {questionAnalysis.length > 0 && (
                        <div>
                          <h4 className="font-display text-sm font-semibold mb-3 flex items-center gap-2"><Target className="w-4 h-4 text-accent" /> Desempenho por Questão (% acerto)</h4>
                          <ResponsiveContainer width="100%" height={Math.max(180, questionAnalysis.length * 35)}>
                            <BarChart data={questionAnalysis} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
                              ...
                              <Tooltip contentStyle={{ borderRadius: 8, border: "1px solid hsl(220,13%,91%)", fontSize: 12 }} formatter={(value: number, _name: string, props: any) => [`${value}% (${props.payload.correct}/${props.payload.total})`, "Taxa de acerto"]} labelFormatter={(label) => { const item = questionAnalysis.find(q => q.question === label); return item?.fullQuestion || label; }} />
                              <Bar dataKey="acertos" radius={[0, 4, 4, 0]}>
                                {questionAnalysis.map((q, i) => (<Cell key={i} fill={q.acertos >= 70 ? "hsl(142, 71%, 45%)" : q.acertos >= 40 ? "hsl(38, 92%, 50%)" : "hsl(0, 72%, 51%)"} />))}
                              </Bar>
                            </BarChart>
                          </ResponsiveContainer>
                          {questionAnalysis.filter(q => q.acertos < 50).length > 0 && (
                            <div className="mt-3 bg-destructive/5 border border-destructive/20 rounded-lg p-3">
                              ...
                              {questionAnalysis.filter(q => q.acertos < 50).map((q, i) => (<p key={i} className="text-xs text-muted-foreground">• {q.fullQuestion} — {q.acertos}% de acerto</p>))}
                            </div>
                          )}
                        </div>
                      )}
        ...
        <div className="text-center py-20">
          <div className="w-16 h-16 rounded-2xl bg-secondary flex items-center justify-center mx-auto mb-4"><BarChart3 className="w-8 h-8 text-muted-foreground" /></div>
  ...
  return (
    <UpgradeGate allowed={canUseAdvancedAnalytics()} featureName="Analytics Avançado" planRequired="Professor">
      {analyticsContent}
    </UpgradeGate>
  );
};
