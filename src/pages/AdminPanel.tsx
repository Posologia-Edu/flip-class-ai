...
import { ShieldCheck, UserCheck, UserX, Clock, Users, BookOpen, BarChart3, Send, Mail, Trash2, Key, CreditCard, Calendar, Globe, Rocket } from "lucide-react";
...
const AdminPanel = () => {
  ...
  const [invitePlan, setInvitePlan] = useState("institutional");
  ...
  const fetchData = useCallback(async () => {
    ...
    setStats({ totalTeachers: approvedCount, totalRooms: roomsRes.count || 0, totalStudents: 0, totalSessions: sessionsRes.count || 0 });
  ...
  const fetchInvites = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("admin-invite", { body: { action: "list_invites" } });
  ...
  const approveTeacher = async (teacher: PendingTeacher) => {
    ...
    const { error } = await supabase.from("profiles").update({ approval_status: "approved", approved_at: new Date().toISOString(), approved_by: user?.id }).eq("id", teacher.id);
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); }
    else { try { await supabase.functions.invoke("send-approval-email", { body: { userId: teacher.user_id, approved: true } }); } catch {} toast({ title: "Professor aprovado!" }); fetchData(); }
    setProcessing(null);
  };
  ...
  const rejectTeacher = async (teacher: PendingTeacher) => {
    ...
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); }
    else { try { await supabase.functions.invoke("send-approval-email", { body: { userId: teacher.user_id, approved: false } }); } catch {} toast({ title: "Cadastro rejeitado." }); fetchData(); }
    setProcessing(null);
  };

  const sendInvite = async () => {
    const trimmed = inviteEmail.trim().toLowerCase();
    if (!trimmed || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmed)) { toast({ title: "Email inválido", variant: "destructive" }); return; }
    setSendingInvite(true);
    try {
      const { data, error } = await supabase.functions.invoke("admin-invite", {
        body: { action: "invite", email: trimmed, granted_plan: invitePlan },
      });
      if (error || data?.error) { toast({ title: "Erro", description: data?.error || "Falha ao enviar convite", variant: "destructive" }); }
      else { toast({ title: "Convite enviado!", description: `Email enviado para ${trimmed}` }); setInviteEmail(""); fetchInvites(); }
  ...
  const revokeInvite = async (invite: Invite) => {
    ...
    try {
      ...
      if (error || data?.error) { toast({ title: "Erro", description: data?.error || "Falha ao revogar", variant: "destructive" }); }
      else { toast({ title: "Acesso revogado!", description: `Convite para ${invite.email} foi removido.` }); fetchInvites(); fetchData(); }
    } catch (err: any) { toast({ title: "Erro", description: err.message, variant: "destructive" }); }
    setProcessing(null);
  };
  ...
  const planLabel = (plan: string | undefined) => {
    ...
    if (plan === "professor") return "Professor";
    if (plan === "institutional") return "Institucional";
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-8">
        <h1 className="font-display text-2xl font-bold text-foreground flex items-center gap-2">
          <ShieldCheck className="w-6 h-6 text-primary" /> Painel de Administração
        </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><Users className="w-4 h-4" /> Professores</div>
          <p className="font-display text-2xl font-bold text-foreground">{stats.totalTeachers}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><Clock className="w-4 h-4" /> Pendentes</div>
          <p className="font-display text-2xl font-bold text-accent">{pending.length}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><BookOpen className="w-4 h-4" /> Salas</div>
          <p className="font-display text-2xl font-bold text-foreground">{stats.totalRooms}</p>
        </div>
        <div className="bg-card border border-border rounded-xl p-4">
          <div className="flex items-center gap-2 text-muted-foreground text-sm mb-1"><BarChart3 className="w-4 h-4" /> Sessões</div>
      ...
      <Tabs defaultValue="updates" className="space-y-6">
        <TabsList className="flex-wrap">
          <TabsTrigger value="updates"><Rocket className="w-4 h-4 mr-1" /> Atualizações</TabsTrigger>
          ...
          <div className="space-y-6">
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                O convidado receberá um email com link para criar senha e terá acesso de acordo com o plano selecionado.
              </p>
              <div className="flex gap-2">
                ...
                <Button onClick={sendInvite} disabled={sendingInvite}>
                  <Mail className="w-4 h-4 mr-1" /> {sendingInvite ? "Enviando..." : "Enviar"}
                </Button>
              </div>
            </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {" · Plano: "}<span className="font-medium">{planLabel(inv.granted_plan)}</span>
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${inv.status === "active" ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-500"}`}>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              <AlertDialogTitle>{inv.status === "active" ? "Revogar acesso" : "Revogar convite"}</AlertDialogTitle>
                              <AlertDialogDescription>
                                {inv.status === "active"
                                  ? `O usuário ${inv.email} perderá o acesso imediatamente e será bloqueado do sistema.`
                                  : `Tem certeza que deseja revogar o convite para ${inv.email}?`}
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => revokeInvite(inv)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                {inv.status === "active" ? "Revogar Acesso" : "Revogar"}
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
        ...
        <TabsContent value="api-keys">
          <div className="bg-card border border-border rounded-xl p-6"><AiApiKeysManager /></div>
        </TabsContent>

        <TabsContent value="pending">
          {pending.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-8 text-center text-muted-foreground">Nenhum cadastro pendente.</div>
          ) : (
              ...
              {pending.map((t) => (
                <div key={t.id} className="bg-card border-2 border-accent/30 rounded-xl p-5 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Cadastrado em {new Date(t.created_at).toLocaleDateString("pt-BR")}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => approveTeacher(t)} disabled={processing === t.id}><UserCheck className="w-4 h-4 mr-1" /> Aprovar</Button>
                    <Button size="sm" variant="outline" onClick={() => rejectTeacher(t)} disabled={processing === t.id} className="border-destructive text-destructive hover:bg-destructive/10"><UserX className="w-4 h-4 mr-1" /> Rejeitar</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <TabsContent value="approved">
          {approved.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-6 text-center text-muted-foreground">Nenhum professor aprovado ainda.</div>
          ) : (
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-secondary">
                ...
                <tbody>
                  {approved.map((t) => (
                    <tr key={t.id} className="border-t border-border">
                      ...
                      <td className="px-4 py-3"><span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-primary/10 text-primary">Aprovado</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <TabsContent value="rejected">
          {rejected.length === 0 ? (
            <div className="bg-card border border-border rounded-xl p-6 text-center text-muted-foreground">Nenhum cadastro rejeitado.</div>
          ) : (
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-secondary"><tr><th className="text-left px-4 py-3 font-medium text-muted-foreground">Nome</th><th className="text-left px-4 py-3 font-medium text-muted-foreground">Data</th></tr></thead>
                <tbody>
                  {rejected.map((t) => (
                    <tr key={t.id} className="border-t border-border">
